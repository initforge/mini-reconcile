                    ...
                    <tr key={record.id} className={`hover:bg-slate-100 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}>
                      ...
                      {showMerchantColumn && (
                        <td className="px-2 sm:px-3 md:px-4 lg:px-6 py-2 sm:py-3 md:py-4 whitespace-nowrap text-[10px] sm:text-xs md:text-sm text-slate-900 bg-green-50/80 border-r border-green-100">
                          {record.transactionCode || record.merchantsFileData?.['Mã trừ tiền_Mã chuẩn chi'] || record.merchantsFileData?.['Mã trừ tiền Mã chuẩn chi'] || record.merchantsFileData?.['Mã chuẩn chi'] || record.merchantsFileData?.['mã chuẩn chi'] ? (
                            <span className="font-mono font-semibold text-slate-800">{record.transactionCode || String(record.merchantsFileData['Mã trừ tiền_Mã chuẩn chi'] || record.merchantsFileData['Mã trừ tiền Mã chuẩn chi'] || record.merchantsFileData['Mã chuẩn chi'] || record.merchantsFileData['mã chuẩn chi'] || '')}</span>
                          ) : <span className="text-slate-400">-</span>}
                        </td>
                      )}
                      {/* 8. Số điện thoại */}
